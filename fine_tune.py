...
def main():
    # 加载此前修改全连接层后训练的模型（与train_fc.py最大的不同之一）
    ...
    data_hflip = transforms.Compose([transforms.ToTensor(),
                                     transforms.Resize(args.resize_size),
                                     transforms.CenterCrop(args.input_size),
                                     transforms.RandomHorizontalFlip(1),
                                     # 这里是自行计算的GTSRB训练集的均值和方差
    ...
    data_vflip = transforms.Compose([transforms.ToTensor(),
                                     transforms.Resize(args.resize_size),
                                     transforms.CenterCrop(args.input_size),
                                     transforms.RandomVerticalFlip(1),
                                     # 这里是自行计算的GTSRB训练集的均值和方差
    ...
    data_grayscale = transforms.Compose([transforms.ToTensor(),
                                         transforms.Resize(args.resize_size),
                                         transforms.CenterCrop(args.input_size),
    ...
    # # 可视化查看单个样本的属性
    # sample = train_dataset[15000][0]
    # sample_id = train_dataset[15000][1]
    # # sample = valid_dataset[0][0]
    # # sample_id = valid_dataset[0][1]
    # # print("第一个样本：", sample)
    # sample_img = sample.numpy().transpose(1, 2, 0)
    # # 将图像从BGR转换为RGB颜色空间
    # sample_img = cv2.cvtColor(sample_img, cv2.COLOR_BGR2RGB)
    # cv2.imshow(str(sample_id), sample_img)
    # cv2.waitKey(0)
